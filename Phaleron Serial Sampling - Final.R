...
#R Script for “From the Boy Comes the Man” : 
#Exploring Dietary Variability in Distinct Burial 
#Contexts at the Archaic Phaleron Burial Ground
...
weaning_age_summary<-summary(weaning_ages)
  ...
  ##FIGURE 2: INDIVIDUAL WEANING CURVES####
...
age.comparison.C
  ...
  ##FIGURE 3: SOCIAL AGE COMPARISON####
  ...
  weaning <- infant_diet %>%
    ...
    dietary_contributions <- ggplot() + 
      ...
      scale_alpha_continuous(
        name = "Age of Serial Sample",
        range = c(0.3, 1),
        breaks = c(0.5, 1, 1.5, 2, 2.5, 3),
        labels = c("0–0.5", "0.5–1", "1–1.5", "1.5–2", "2–2.5", "2.5–3")
      ) +
      scale_shape_manual(
        name = "Individual",
        values = c(
      ...
      scale_fill_manual(
        ...
        values = c(
          "IV_281" = "#2C728E", 
          "IV_734" = "#3B9AB2", 
          "Other Individuals" = "#A6DCE3"
        )) +
    guides(
      fill  = "none",
      alpha = guide_legend(title = "Age of Serial Sample", override.aes = list(shape = 15)),
...
####MORTUARY ANALYSIS###########################################################
       ...
       ##for the visualization, I decided to keep single shackled and mass burial 
       ##separate, but I want to combine them for the stats tests since there is
       ##only 1 single shackled individual. I called this category "restrained" 
       ##since they all have restraints. 
       M1_data <- M1_data %>%
         mutate(
           burial.type.sup.1 = case_when(
             burial.type.sup %in% c("single shackled", "mass burial") ~ "restrained",
             TRUE ~ burial.type.sup   # keep all other burial types the same
           )
         )
       infant_diet <- infant_diet %>%
         mutate(
    ...
    ##KRUSKALL-WALLIS####
       burial.type.sup.1.kruskal.N <- kruskal.test(d15N.Air ~ burial.type.sup.1, data = M1_data)
       burial.type.sup.1.kruskal.C <- kruskal.test(d13C ~ burial.type.sup.1, data = M1_data)
       
       burial.type.sup.1.infant.kruskal.N <- kruskal.test(d15N.Air ~ burial.type.sup.1, data = infant_diet)
       burial.type.sup.1.infant.kruskal.C <- kruskal.test(d13C ~ burial.type.sup.1, data = infant_diet)
       
       burial.type.sup.1.children.kruskal.N <- kruskal.test(d15N.Air ~ burial.type.sup.1, data = child_diet)
       burial.type.sup.1.children.kruskal.C <- kruskal.test(d13C ~ burial.type.sup.1, data = child_diet)
       
       kruskal.table.burial.type.sup.1 <- tibble(
         test = c(
           "Overall d15N",
           "Overall d13C",
           "Infant d15N",
         ...
         H_statistic = c(
           burial.type.sup.1.kruskal.N$statistic,
           burial.type.sup.1.kruskal.C$statistic,
           burial.type.sup.1.infant.kruskal.N$statistic,
           burial.type.sup.1.infant.kruskal.C$statistic,
           burial.type.sup.1.children.kruskal.N$statistic,
           burial.type.sup.1.children.kruskal.C$statistic
         ),
         df = c(
           burial.type.sup.1.kruskal.N$parameter,
           burial.type.sup.1.kruskal.C$parameter,
           burial.type.sup.1.infant.kruskal.N$parameter,
           burial.type.sup.1.infant.kruskal.C$parameter,
           burial.type.sup.1.children.kruskal.N$parameter,
           burial.type.sup.1.children.kruskal.C$parameter
         ),
         p_value = c(
           burial.type.sup.1.kruskal.N$p.value,
           burial.type.sup.1.kruskal.C$p.value,
           burial.type.sup.1.infant.kruskal.N$p.value,
           burial.type.sup.1.infant.kruskal.C$p.value,
           burial.type.sup.1.children.kruskal.N$p.value,
           burial.type.sup.1.children.kruskal.C$p.value
         )
       )
       
       kruskal.table.burial.type.sup.1
       write.csv(kruskal.table.burial.type.sup.1, "kruskal.table.burial.type.sup.1.csv")
       
    ##PAIRWISE WILCOX####
       #Overall diet
       burial.type.sup.1.pairwise.wilcox.N <- M1_data %>%
         pairwise_wilcox_test(
           formula = d15N.Air ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       burial.type.sup.1.pairwise.wilcox.C <- M1_data %>%
         pairwise_wilcox_test(
           formula = d13C ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       #Infants
       infant.burial.type.sup.1.pairwise.wilcox.N <- infant_diet %>%
         pairwise_wilcox_test(
           formula = d15N.Air ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       infant.burial.type.sup.1.pairwise.wilcox.C <- infant_diet %>%
         pairwise_wilcox_test(
           formula = d13C ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       #Children
       child.burial.type.sup.1.pairwise.wilcox.N <- child_diet %>%
         pairwise_wilcox_test(
           formula = d15N.Air ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       child.burial.type.sup.1.pairwise.wilcox.C <- child_diet %>%
         pairwise_wilcox_test(
           formula = d13C ~ burial.type.sup.1,
           p.adjust.method = "holm")
       
       #Results
       pairwise.wilcox.table.burial.type.sup.1 <- bind_rows(
         burial.type.sup.1.pairwise.wilcox.N %>% mutate(test = "Overall d15N"),
         burial.type.sup.1.pairwise.wilcox.C %>% mutate(test = "Overall d13C"),
         infant.burial.type.sup.1.pairwise.wilcox.N %>% mutate(test = "Infant d15N"),
         infant.burial.type.sup.1.pairwise.wilcox.C %>% mutate(test = "Infant d13C"),
         child.burial.type.sup.1.pairwise.wilcox.N %>% mutate(test = "Child d15N"),
         child.burial.type.sup.1.pairwise.wilcox.C %>% mutate(test = "Child d13C")) %>%
         select(test, group1, group2, W = statistic, p.adj, p.adj.signif)
       
       pairwise.wilcox.table.burial.type.sup.1
       write.csv(pairwise.wilcox.table.burial.type.sup.1, "pairwise.wilcox.table.burial.type.sup.1.csv")
